...
test('inline', (t) => {
    ...
    t.notOk(
        postcss()
        .use(url({ url: 'inline', maxSize: 0.0001 }))
        .process(read('fixtures/inline-from'), { from: 'test/fixtures/here' })
        .css.match(/;base64/),
...
test('custom', (t) => {
    let declOk = false;
    const opts = {
        url(URL, dir, options, result, decl) {
            if (!declOk) {
                t.ok(decl,
...
test('ignore absolute urls, data uris, or hashes', (t) => {
    compareFixtures(
        t,
        'absolute-urls',
...
function testCopy(t, opts, postcssOpts) {
    ...
    const patterns = {
        ...
        copyXXHashPixel16: new RegExp(`"${assetsPath}[a-z0-9]{16}\\.png"`),
        copyXXHashParamsPixel16: new RegExp(`"${assetsPath}[a-z0-9]{16}\\.png\\?v=1\\.1\\#iefix"`),
        copyCustomHashPixel16: new RegExp(`"${assetsPath}123\\.png"`)
    };
...
test('copy-without-assetsPath', (t) => {
    const opts = {
        url: 'copy'
    };
...
test('copy-when-inline-fallback', (t) => {
    const opts = {
        url: 'inline',
        maxSize: 0.0001,
        fallback: 'copy',
...
test('function-when-inline-fallback', (t) => {
    const opts = {
        url: 'inline',
        maxSize: 0.0001,
        fallback() {
